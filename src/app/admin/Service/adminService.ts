...
export class adminService{
  ...
  constructor(private http:HttpClient){}
    ...
    addNewFlight(flightModel: FlightModel) :Observable<FlightModel>{
      ...
      return this.http.post<FlightModel>("http://localhost:8085/Admin/addFlights", flightModel, {headers: {"Authorization": "Bearer "+this.token}});
    }
    ...
    getFlights(flightName:String) : Observable<FlightModel[]>{
        ...
        return this.http.get<FlightModel[]>("http://localhost:8085/Flight/GetAllFlights?FlightName="+flightName);
    }

    deleteFlight(flightId:Number){
      this.token = localStorage.getItem("token");
      return this.http.delete("http://localhost:8085/Admin/deleteFlight/"+flightId, {headers: {"Authorization": "Bearer "+this.token}});
  }
}